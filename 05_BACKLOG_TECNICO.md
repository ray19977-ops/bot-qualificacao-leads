...
| INFRA-03 | Setup do servidor web | Configurar FastAPI para servir a interface estática (`static/`) e expor a rota `POST /chat` (ainda sem lógica de LLM) — Arquitetura Seção 5 | Servidor sobe com `uvicorn`, serve a página inicial e a rota `/chat` responde (mesmo que com stub) | INFRA-01 | sim | concluido |
